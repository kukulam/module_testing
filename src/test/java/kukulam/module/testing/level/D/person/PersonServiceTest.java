...
public class PersonServiceTest {
    ...
    @Test
    public void shouldFindAllPersonOlderThanGivenAgeWithOwnAssertion() {
        // given
        int givenAge = 30;
        ...
        // then
        PersonsAgeAssert.assertThat(result).hasAgeOlderThan(givenAge);
    }

    static class PersonsAgeAssert extends AbstractAssert<PersonsAgeAssert, List<Person>> {
        public PersonsAgeAssert(List<Person> people) {
            super(people, PersonsAgeAssert.class);
        }

        public static PersonsAgeAssert assertThat(List<Person> actual) {
            return new PersonsAgeAssert(actual);
        }

        public PersonsAgeAssert hasAgeOlderThan(int age) {
            isNotNull();

            actual.forEach(person -> {
                if (person.getAge() <= age)
                    failWithMessage("Expected person's age to be older than <%s> but was <%s>", age, person.getAge());
            });
